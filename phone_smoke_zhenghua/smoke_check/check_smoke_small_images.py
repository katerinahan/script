...
def crop_to_small_images(json_dir, img_dir, save_dir, class_type, flag_train):
  ...
  for json_file_name in glob.glob(json_dir + '/*.json'):
    ...
    for line in lines:
      ...
      # get the face box
      crop_box = image_arrt.driverFaceBox
      
      # get samples
      save_sample_path = save_subdir
      if image_arrt.isSmoking == 1:
        save_sample_path = save_subdir + '/smoke'
        crop_train_test_samples(img, crop_box, save_sample_path, image_arrt.imgName, flag_train)
      elif image_arrt.isSmoking == 0:
        save_sample_path = save_subdir + '/nosmoke'
        crop_train_test_samples(img, crop_box, save_sample_path, image_arrt.imgName, flag_train)
      else:
        print ('Ignored: ', image_arrt.imgName)
        continue
